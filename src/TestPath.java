...
public class TestPath extends LockedTestCase {
	...
	public void test() {
		// asString returns a string for the expression, "null" (if null) or the name of the exception thrown
		// tn prints as n.
		t1.dependsOn(t2);
		t3.dependsOn(t1);
		t5.dependsOn(t3);
	...
	public void test54() {
		...
		path = path.append(t[5]);
		assertEquals(Arrays.asList(t[2],t[3],t[1],t[2],t[4],t[5]), Arrays.asList(path.toArray()));
	}
